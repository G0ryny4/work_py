...
def rnd(): # создание случайного число от 0 до 100
...
def k_mn(k):  # создание многочлена в виде строки
    if 'x' in k:
        i = k.find('x')
        num = int(k[:i])
    return num
...
koef1 = create_mn(k1)
koef2 = create_mn(k2)
write_file("file54_1.txt", create_str(koef1))
write_file("file54_2.txt", create_str(koef2))

with open('file54_1.txt', 'r') as data: # нахождение суммы многочлена
    st1 = data.readlines()
with open('file54_2.txt', 'r') as data:
    st2 = data.readlines()
...
write_file("file54_res.txt", create_str(lst_new))
with open('file54_res.txt', 'r') as data:
    st3 = data.readlines()
